...
长度限制为 [1,64]
    ...
    2.
以字母、数字或下划线开头，后面可跟字母、数字、下划线或连字符（-）的字符串
...
```
# 第一次请求，不携带Header，服务端会自动生成Session ID
...
```
# 使用提取的Session ID发起后续请求
curl -v -H "mySessionId: <从响应中提取的Session ID>" http://your-function-url/your-path
```
