...
all in one 模板将 browser 模板与 code-interpreter 模板能力合并，提供一个统一的云端隔离执行环境。all in one 沙箱既能做浏览器级自动化（CDP / VNC / 录制），也能做代码执行、文件管理与交互式终端，是 AI Agent 的"眼睛 + 大脑 + 手"。
...
| 代码执行 | 集成 code-interpreter 模板能力，支持 Python/JavaScript 代码执行、上下文管理、文件系统操作 |
...
> **说明**
>
> all in one 模板的默认 CPU 和内存规格高于 code-interpreter 模板和 browser 模板，因为需要同时支撑浏览器运行时和代码执行环境。
...
- [code-interpreter 模板](code-interpreter-模板.md)（仅需代码执行能力时选择）
